...
describes what users get. Pick a format with `-o`, or let mavencentral choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mavencentral <command> -o table     # a rounded, color-aware grid for reading
mavencentral <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
mavencentral <command> -o jsonl     # one JSON object per line, for piping
mavencentral <command> -o json      # a single JSON array
mavencentral <command> -o csv       # spreadsheet friendly
mavencentral <command> -o tsv       # tab-separated
mavencentral <command> -o url       # just the URL column
mavencentral <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
mavencentral <command> --fields id,title,url
```
...
```bash
mavencentral <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mavencentral <command>            # a table, because this is a terminal
mavencentral <command> | wc -l    # JSONL, because this is a pipe
```
